...
## 2026-09-07 — CU1 미배정자 셀 배정 및 태신자셀 신설
- 사전 감사에서 활성 CU1 셀 미배정자 13명, 기존 활성 `선웅셀`·`현진셀`·`새신자셀`, `태신자셀` 부재를 확인했다.
- 김효정은 이미 활성 `team_leader`이고 CU1 `is_team_leader=true`였다. 계정/권한은 변경하지 않고 셀 미배정으로 유지했다.
- `scripts/ops-2026-09-07-assign-cu1-unassigned-members.sql`을 작성해 프로덕션에서 `ROLLBACK` 드라이런을 통과한 뒤 실제 트랜잭션을 적용했다.
- 김선웅→선웅셀, 이현진→현진셀. 신규 태신자셀에는 구현서, 김민호, 김영효, 김지솔, 박수빈, 송준선, 신원주, 우현승, 장성재, 현수빈을 배정했다.
...
## 2026-09-08 — 최신 1청년부 명단·셀 동기화
- `1청년부 전체 명단 (26.09.08).xlsx` 50명과 프로덕션 CU1 50명을 대조하고 `scripts/ops-2026-09-08-sync-cu1-roster-from-xlsx.sql`을 작성했다.
- 프로덕션 `ROLLBACK` 드라이런 통과 후 실제 트랜잭션 적용 완료.
- 신희준 신규 등록·선웅셀 배정, 성모셀 신설·정성모 배정, 김민호→다희셀, 박수빈→민아셀, 정시후·정은재→태신자셀을 반영했다.
- 한수연은 두 사람으로 유지했다. 한수연a는 현진셀+CU 워십팀과 출결 12건을 보존했고, 한수연b는 태희셀과 출결 2건을 유지했다. 제거한 행은 한수연a의 이력 없는 후발 중복 행뿐이다.
- 박승조 생년월일을 `1998-01-27`로 정정했다. 신희준의 불완전 생년월일과 빈 연락처는 추측하지 않고 null로 두었다.
