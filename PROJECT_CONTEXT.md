...
- Report form local draft backups are now versioned and cleared after successful final submission, so an already-submitted report id is not restored into a future new-report attempt.
...
## 2026-08-21 Update - Audit Phase 0 Part 1 (P0-2 / P0-4 / P0-5)
- First implementation session against the 2026-08-21 architecture audit. Three of the eight P0 items are done; the other five are blocked and listed below.
- **P0-2 (encoding)**: Restored the Korean text in `src/components/settings/CellManager.tsx`, `src/app/(dashboard)/members/bulk-photos/page.tsx`, and `src/app/(dashboard)/members/[id]/edit/page.tsx`. The corruption entered in commit `9a55fa0`, which also carried genuine refactoring, so only the broken strings were replaced rather than reverting the files. `npm run docs:check` now fails on CJK ideographs or `?`+Hangul anywhere under `src`/`public`, and `.gitattributes` normalizes text files to LF.
- **P0-4 (print XSS)**: All interpolations in the three report print HTML generators are escaped, rich-text fields go through `DOMPurify.sanitize()` to match on-screen rendering, and `printHtmlInIframe` now uses `iframe.srcdoc` with `sandbox="allow-same-origin allow-modals"` so injected handlers cannot execute on the app origin. The inline print `<script>` was removed; the parent `onload` handler triggers printing.
