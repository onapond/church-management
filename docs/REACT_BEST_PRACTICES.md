...
### URL 상태 파생 (rerender-derived-state-no-effect)

```tsx
// ❌ Bad - useEffect로 URL 동기화
...
### 모달 컴포넌트 분리 (rerender-memo)

```tsx
// ❌ Bad - 인라인 모달 (부모 리렌더 시 함께 리렌더)
...
const handleCancel = useCallback(() => setDeleteTarget(null), [])
<button onClick={handleCancel}>취소</button>
```

### 적용된 컴포넌트 예시
- `src/components/members/MemberList.tsx` - URL 파생 상태, 모달 분리, 메모이제이션
