...
3. 필터를 사용하여 원하는 교인을 찾을 수 있습니다:
   - **부서 필터**: 드롭다운에서 부서 선택
   - **셀 필터**: 1청년부 선택 시에만 나타남 (민아셀, 선웅셀, 다희셀, 현진셀, 태희셀, 새신자셀, 채현셀, 태신자셀, 성모셀)
...
- 2026년 9월 8일 프로덕션 Chrome에서 실제 인쇄 창이 정상적으로 열리는 것을 확인했습니다.
...
## 셀장보고서 출석 연계 변경 (2026-09-06)
- 셀장보고서의 **셀원 출석**에는 **예배**와 **모임**이 별도 열로 표시됩니다. 각 셀원에 대해 두 항목을 각각 체크하세요.
- **예배 전체**, **모임 전체**, **초기화** 버튼으로 한 종류씩 빠르게 변경할 수 있습니다.
- 임시저장 또는 제출 버튼으로 명시적으로 저장하면 보고서 출석이 출결 관리와 함께 저장되고, 상세 화면에도 두 종류가 분리되어 표시됩니다.
- 백그라운드 자동 임시저장은 입력 중인 출석을 임의로 덮어쓰지 않습니다.
- 과거 셀장보고서에 개인 출석 연결이 없으면 본문만 수정할 때 기존 요약을 유지합니다. 출석을 직접 바꿔 저장한 시점부터 새 개인 출석 기준으로 전환됩니다.
- 셀장보고서 취합으로 만드는 주차보고서는 선택한 보고서의 개인 출석을 기준으로 재적·예배·모임 수를 계산합니다.
- 출결 관리에서 나중에 수동으로 고친 값은 수동 수정으로 기록되며, 해당 보고서를 삭제해도 남습니다.

## 1청년 다희셀·태희셀 명단 정정 (2026-09-07)
- 다희셀 출석부가 비어 있고 다희셀 교인이 태희셀에 함께 보이던 문제를 정정했습니다.
- 다희셀에는 김동혁, 김은수, 도지수, 이다희, 장미화가 표시됩니다.
- 태희셀에는 강태웅, 김민지, 박승조, 이태희, 조민정, 한수연b가 표시됩니다.
- 출결 관리 화면이 열려 있었다면 새로고침하거나 셀을 다시 선택해 최신 명단을 불러오세요.

## 1청년 미배정자 셀 배정 (2026-09-07)
- 김선웅은 기존 선웅셀, 이현진은 기존 현진셀 출석부에 표시됩니다.
- 태신자셀이 새로 추가되었으며 구현서, 김민호, 김영효, 김지솔, 박수빈, 송준선, 신원주, 우현승, 장성재, 현수빈이 표시됩니다.
- 김효정 팀장은 셀 미배정 상태로 유지됩니다.
- 기존 새신자셀은 태신자셀과 별도로 유지됩니다.
- 화면이 열려 있었다면 새로고침하거나 셀을 다시 선택해 최신 명단을 불러오세요.

## 1청년 최신 명단·셀 반영 (2026-09-08)
- 1청년 명단은 2026-09-08 기준 활성 교인 50명과 활성 셀 9개로 갱신되었습니다.
- `한수연a`는 현진셀에 표시되며 CU 워십팀 소속도 유지됩니다. `한수연b`는 다른 교인으로 태희셀에 표시됩니다.
- 신희준은 선웅셀, 정성모는 새로 만든 성모셀에 표시됩니다.
- 김민호는 다희셀, 박수빈은 민아셀, 정시후와 정은재는 태신자셀에 표시됩니다.
- 김효정 팀장은 기존 결정대로 셀 미배정 상태를 유지합니다.
- 화면이 열려 있었다면 새로고침하거나 셀을 다시 선택해 최신 명단을 불러오세요.
